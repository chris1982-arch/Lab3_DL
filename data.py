...
def image_level_split(
    ...
    """
    unique_images = df["image_id"].drop_duplicates().sample(
...
def _extract_captions(example: dict) -> List[str]:
    """Pulls all caption strings out of a HuggingFace row, regardless of
    whether the field is named 'caption', 'captions', 'sentences', etc."""
    for key in ("captions", "sentences", "caption"):
        if key in example:
            value = example[key]
            if isinstance(value, list):
                # Some datasets wrap each caption in a dict {"raw": "..."}.
                out = []
                for v in value:
